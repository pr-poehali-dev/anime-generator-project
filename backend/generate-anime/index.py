...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    episode_openings = rng.sample(opening_songs, min(12, len(opening_songs)))
    episode_endings = rng.sample(ending_songs, min(12, len(ending_songs)))
    ...
    for i, template in enumerate(episode_templates, 1):
        ...
        full_story = f"""Серия начинается с того, что {hero_name} {['просыпается в новый день', 'готовится к важному событию', 'размышляет о произошедшем', 'получает важное сообщение'][i % 4]}. 

{template.format(hero=hero_name, mentor=mentor_name, rival=rival_name)}. События разворачиваются стремительно, когда {hero_name} {['встречает загадочного незнакомца', 'обнаруживает древний артефакт', 'узнаёт страшную правду', 'принимает сложное решение'][i % 4]}.

Основная часть эпизода посвящена {['развитию персонажей и их взаимоотношений', 'напряжённым боевым сценам', 'раскрытию тайн прошлого', 'эмоциональным моментам и драме'][i % 4]}. {mentor_name if i > 2 else rival_name} играет ключевую роль в {['помощи главному герою', 'создании препятствий', 'раскрытии важной информации', 'изменении хода событий'][i % 4]}.

Кульминация серии происходит, когда {hero_name} {['сражается с могущественным противником', 'делает важное открытие', 'жертвует чем-то важным', 'спасает союзников'][i % 4]}. Это приводит к {['неожиданному повороту сюжета', 'важному развитию характера', 'новым вопросам и загадкам', 'эмоциональному разрешению конфликта'][i % 4]}.

Серия завершается {['интригующим cliffhanger', 'спокойной сценой подведения итогов', 'намёком на будущие события', 'трогательным моментом между персонажами'][i % 4]}, оставляя зрителя в предвкушении следующего эпизода."""
        
        key_moments = [
            f"{hero_name} {['обнаруживает свою скрытую силу', 'сталкивается с внутренними страхами', 'принимает судьбоносное решение', 'встречает важного союзника'][i % 4]}",
            f"Драматическая сцена с {[mentor_name, rival_name, characters[3]['name'].split()[0] if len(characters) > 3 else 'персонажем'][i % 3]}",
            f"{['Эпичная битва', 'Эмоциональная конфронтация', 'Раскрытие секрета', 'Неожиданный поворот'][i % 4]} в {['древнем храме', 'футуристическом городе', 'школьном кампусе', 'таинственном лесу'][i % 4]}",
            f"Развитие отношений между персонажами и {['новое понимание', 'усиление конфликта', 'момент прощения', 'клятва верности'][i % 4]}",
            f"Предвестие будущих событий и {['появление нового врага', 'раскрытие древнего пророчества', 'утрата важного артефакта', 'предательство союзника'][i % 4]}"
        ]
